const computer = (react) => {
  ...
  const twoInFirstDiagonal = diagonalArray[0].filter(
    (symbol) => symbol === "X"
  );
  const twoInSecondDiagonal = diagonalArray[1].filter(
    (symbol) => symbol === "X"
  );
  ...
  const nullInFirstColum = verticalArrays[0].filter(
    (symbol) => symbol === null
  );
  const nullInSecondColum = verticalArrays[1].filter(
    (symbol) => symbol === null
  );
  const nullInThirdColum = verticalArrays[2].filter(
    (symbol) => symbol === null
  );
  ...
  if (isTwoInFirstRow && isNullInFirstRow) {
    updatedBoxes[0].splice(findIndexOfNull1, 1, "0");
  } else if (isTwoInSecondRow && isNullInSecondRow) {
    updatedBoxes[1].splice(findIndexOfNull2, 1, "0");
  } else if (isTwoInThirdRow && isNullInThirdRow) {
    updatedBoxes[2].splice(findIndexOfNull3, 1, "0");
  } else if (isTwoInFirstColum && isNullInFirstColum) {
    verticalArrays[0].splice(findIndexOfNullVertical1, 1, "0");
  } else if (isTwoInSecondColum && isNullInSecondColum) {
    verticalArrays[1].splice(findIndexOfNullVertical2, 1, "0");
  } else if (isTwoInThirdColum && isNullInThirdColum) {
    verticalArrays[2].splice(findIndexOfNullVertical3, 1, "0");
  ...
  if (
    [...updatedBoxes[0], ...updatedBoxes[1], ...updatedBoxes[2]].join() ===
      [...originalBox[0], ...originalBox[1], ...originalBox[2]].join() &&
    [...diagonalArray[0], diagonalArray[1], diagonalArray[2]].join() ===
      [
        ...diagonalCopyArray[0],
        diagonalCopyArray[1],
        diagonalCopyArray[2],
    ...
    [...diagonalArray[0], ...diagonalArray[1]].join() !==
    [...diagonalCopyArray[0], ...diagonalCopyArray[1]].join()
  ) {
  ...
  if (newCount > 4) {
    react.checkforWinner(finalUpdatedBoxes);
  }
  react.setState({
